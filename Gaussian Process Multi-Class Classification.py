...
class GaussianProcessMultiClassifier:
    ...
    def find_mode(self, trainingData, trainingLabels, hyp):
        ...
        aexp = np.zeros ((c * n, 1))
        while(True) :
            A, _, _ = self.calculate_intermediate_values(t, a, Kcs)
            b, K, lodget = A[1], A[3], A[2]
            a = np.dot (K, b)
            prev = cur
            lastLodget = lodget
            s = np.zeros (n)
            #print (cur)
            for j in range (0, n) :
                s[j] = sum (np.exp(a[j:c*n:n]))
            #print (i)
            cur = -0.5 * np.dot(np.transpose(b), a)[0][0] + np.dot(np.transpose(t), a)[0][0] - sum(np.log (s))
    ...
    def calculate_intermediate_values(self, t, a, Kcs):
        ...
        D = np.diag(np.reshape(np.transpose(pi), (c * n)))
        #values
        logdet = 0.0
        Ecs = np.zeros((c, n, n))
        P = np.zeros ((0,n))
        ...
        for i in range (0, c) :
            pick = pi[:, i]
            Dc = np.diag(pick)
            P = np.concatenate((P, Dc), axis = 0)
            Dcq = Dc ** (0.5)
            L = np.linalg.cholesky(np.identity(n) + np.dot(Dcq,np.dot(Kcs[i], Dcq)))
            Ecs[i] = np.dot (Dcq, np.linalg.solve(np.transpose(L), np.linalg.solve(L, Dcq)))
            logdet = logdet + np.sum(np.log(np.diag(L)))
        
        EE = np.sum (Ecs, axis = 0) 
        M = np.linalg.cholesky(EE + (1e-6) * np.identity(n))
        E = self.block_diag (Ecs)
        W = D - np.dot(P, np.transpose(P))
        logdet = logdet + np.sum(np.log(np.diag(M)))
        pi = np.reshape(pi.T, (c * n, 1))
        C = np.dot(W, a) + t - pi
        dd = np.dot(E, np.dot(K, C))
        R = np.zeros((0, n))
        for i in range (0, c) :
            R = np.concatenate((R, np.identity(n)), axis = 0) 
        b = C - dd + np.dot (E, np.dot (R, np.linalg.solve (np.transpose (M), np.linalg.solve(M, np.dot (np.transpose(R), dd)))))
    ...
    def calculate_predictive_distribution(self, datum, pi, Ecs, M, R, tc):
        ...
        c = len(self.legalLabels)
        for i in range(c):
            Ks.append(np.transpose(self.cov_ard(self.hyp[i, :], np.reshape(datum, (1, d)), self.trainingData)))
        
        pi = np.transpose(np.reshape(pi, (c, n)))
        for i in range (0, c) :
            mu[i] = np.dot(np.transpose(tc[:, i] - pi[:, i]), Ks[i])
            f = np.dot (Ecs[i], Ks[i])
            GGG = np.dot (Ecs[i], np.dot (Rc[i], (np.linalg.solve(np.transpose(M), np.linalg.solve(M, np.dot(np.transpose(Rc[i]), f))))))
            
            for j in range (0, c) : 
                cov[i][j] = np.dot(np.transpose(GGG), Ks[j])
            cov[i][i] += self.cov_ard(self.hyp[i, :], np.reshape(datum,(1,d))) - np.dot(np.transpose(f), Ks[i])
